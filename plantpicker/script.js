...
"SCHOOLYEAR": ["1st","2nd"],
"HOUSEPLANTS": ["Houseplant", "My houseplants","Wishlist"],
"NOTE": ["text"]
};
...
function filterPlantsBySearch() {
  ...
  if (!rawInput) return plants.filter((plant) => !(plant["HOUSEPLANTS"]?.includes("Houseplant")));
  ...
  return plants.filter((plant) => {
    const houseplantInSearch = orGroups.some((group) =>
      group.some((tag) => tag === "houseplant")
    );
    if (plant["HOUSEPLANT"]?.includes("Houseplant") && !houseplantInSearch) return false;

    return orGroups.some((group) => {
      return group.every((tag) => {
        ...
        const matchesPlant =
          Object.keys(plant).some((key) => {
            ...
            else return value.toLowerCase() === tagLower;
          }) ||
...
function renderOverlayContent(plant) {
  ...
  for (const key in plant) {
    if (
      ...
      continue;
    if (key === "HOUSEPLANTS") {
      const filteredValues = (plant[key] || []).filter((v) => v !== "Wishlist");
...
function navigatePlant(direction) {
  ...
  currentPlantIndex =
    (currentPlantIndex + direction + filteredPlants.length) % filteredPlants.length;
  currentImageIndex = 0;
  renderOverlayContent(filteredPlants[currentPlantIndex]);
}

function renderPlants(filterText = "") {
  gallery.innerHTML = "";
